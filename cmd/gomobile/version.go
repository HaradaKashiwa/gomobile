...
import (
	...
	"github.com/haradakashiwa/gomobile/internal/sdkpath"
)
...
func runVersion(cmd *command) (err error) {
	// Check this binary matches the version in github.com/haradakashiwa/gomobile/cmd/gomobile
	...
	version, err := func() (string, error) {
		...
		cmd := exec.Command("go", "list", "-f", "{{.Stale}}", "github.com/haradakashiwa/gomobile/cmd/gomobile")
...
func mobileRepoRevision() (rev string, err error) {
	b, err := exec.Command("go", "list", "-f", "{{.Dir}}", "github.com/haradakashiwa/gomobile/app").CombinedOutput()
	if err != nil {
