...
import (
	...
	"github.com/haradakashiwa/gomobile/bind"
	"github.com/haradakashiwa/gomobile/internal/importers"
	"github.com/haradakashiwa/gomobile/internal/importers/java"
...
func genPkg(lang string, p *types.Package, astFiles []*ast.File, allPkg []*types.Package, classes []*java.Class, otypes []*objc.Named, libName string) {
	...
	case "java":
		...
		if p == nil {
			dir, err := packageDir("github.com/haradakashiwa/gomobile/bind")
			if err != nil {
				errorf(`"github.com/haradakashiwa/gomobile/bind" is not found; run go get github.com/haradakashiwa/gomobile/bind: %v`, err)
				return
			}
			repo := filepath.Clean(filepath.Join(dir, "..")) // github.com/haradakashiwa/gomobile directory.
			...
			javaDir, err := packageDir("github.com/haradakashiwa/gomobile/bind/java")
			if err != nil {
				errorf("unable to import bind/java: %v", err)
				return
			}
	...
	case "objc":
		...
		if p == nil {
			// Copy support files
			dir, err := packageDir("github.com/haradakashiwa/gomobile/bind/objc")
			if err != nil {
